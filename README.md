...
在调用接口 `/api/calculate` 计算北京（116.4°E, 39.9°N）2026年3月23日的日出日落时间时，返回结果存在**严重的时间逻辑错误**：
...
```json
{
  ...
  "result": {
    "date": "2026-03-23",
    "sunrise": "18:10",        // ❌ 错误：日出显示为傍晚6点
    "sunset": "06:18",         // ❌ 错误：日落显示为早晨6点
    "solar_noon": "00:14",     // ❌ 错误：正午显示为午夜
    "day_length": 12.14,
    "civil_twilight": {
      "morning": "17:43",      // ❌ 错误：晨光显示为傍晚
      "evening": "06:45"       // ❌ 错误：暮光显示为早晨
    }
  }
}
```
...
**预期正确结果：**
- 日出时间应在早晨 **06:00-06:30** 之间
- 日落时间应在傍晚 **18:00-19:00** 之间
- 太阳正午应在中午 **12:00-12:30** 之间

#### 根本原因分析

经过代码审查，发现原版本 `sunrise_sunset.go` 存在以下三个关键Bug：

**1. 太阳赤纬计算错误（最严重）**
```go
// 原代码错误：使用固定的赤纬0度
cosH := (math.Sin(h0*math.Pi/180) -
    math.Sin(latitude*math.Pi/180)*math.Sin(0*math.Pi/180)) /  // ❌ 固定为0度
    (math.Cos(latitude*math.Pi/180) * math.Cos(0*math.Pi/180))  // ❌ 固定为0度
```
原代码在计算日出日落时使用了固定的太阳赤纬 `0` 度，而实际上太阳赤纬在一年中变化范围约为 **-23.5° 到 +23.5°**（取决于黄赤交角）。3月23日（春分附近）的太阳赤纬应该接近 0° 但不为 0°，且晨昏蒙影计算需要准确的赤纬值。

**2. 时区转换逻辑错误**
```go
// 原代码错误：直接添加时区偏移
hours += timezone // ❌ 没有正确处理经度对应的本地太阳时
```
原代码直接将时区小时数加到UTC时间上，但正确的做法应该是：
- 先计算UTC时间的日出日落
- 然后根据经度和方程时差进行修正
- 最后加上时区偏移得到本地时间
...
```go
// 原代码错误：经度处理不正确
sunriseJD := jd + (720-4*(longitude+H)-0)/1440.0  // ❌ 符号错误
sunsetJD := jd + (720-4*(longitude-H)-0)/1440.0   // ❌ 符号错误
```
原公式中的经度处理存在符号错误，导致日出日落时间计算颠倒。

#### 修复方案

**新增修复接口：**

1. **`/api/calculate-fixed`** - 使用修复后的算法计算日出日落时间
2. **`/api/calculate-compare`** - 对比原版与修复版的结果差异

**核心修复内容：**

1. **正确计算太阳赤纬**：使用基于NOAA标准的太阳位置算法，根据儒略日精确计算太阳赤纬
2. **修正时区转换**：正确处理UTC到本地时间的转换，包括经度修正和方程时差
3. **修复符号错误**：修正日出日落公式中的经度符号处理

**修复后的算法基于以下天文原理：**
- 使用NOAA（美国国家海洋和大气管理局）的太阳能计算公式
- 考虑黄赤交角、地球轨道离心率、太阳中心差等因素
- 计算方程时差（Equation of Time）修正平太阳时与视太阳时的差异
- 正确处理不同纬度、海拔对日出日落的影响
  ...
  "differences": [
    {
      ...
      "original": "18:10",
      "fixed": "06:14",
      "description": "日出时间存在显著差异，原版显示为傍晚时间",
      "is_critical": true
    },
    {
      "field": "Sunset",
      "original": "06:18",
      "fixed": "18:22",
      "description": "日落时间存在显著差异，原版显示为早晨时间",
      "is_critical": true
    }
  ],
  "summary": "发现6处差异，其中4处为关键字段（日出/日落/正午时间），确认原版存在Bug",
  "original_bug_description": "原版存在时间逻辑错误：日出显示为18:10（傍晚），日落显示为06:18（早晨）",
  "fix_explanation": "修复内容：1) 使用正确的太阳赤纬计算；2) 修正时区转换逻辑；3) 正确处理经度符号和方程时差"
}
```
...
```json
{
  ...
  "result": {
    "date": "2026-03-23",
    "sunrise": "06:14",        // ✅ 正确：早晨日出
    "sunset": "18:22",         // ✅ 正确：傍晚日落
    "solar_noon": "12:18",     // ✅ 正确：中午正午
    "day_length": 12.13,
    "civil_twilight": {
      "morning": "05:47",      // ✅ 正确：晨光在日出前
      "evening": "18:49"       // ✅ 正确：暮光在日落后
    },
    "nautical_twilight": {
      "morning": "05:15",
      "evening": "19:21"
...
3. **计算日出日落时间**
   - 确定日出日落时的天顶角（标准：-0.8333°）
   - 根据海拔修正天顶角
   - 计算时角：cos(H) = (cos(z) - sin(φ)sin(δ)) / (cos(φ)cos(δ))
   - 计算方程时差（EOT）
   - 计算太阳正午的本地视太阳时
   - 转换为UTC时间
   - 计算日出日落UTC时间
