...
type ODEParams struct {
	...
	TimeRange    float64 `json:	ime_range"`    // 时间范围
...
type ODEResult struct {
	...
	TimePoints    []float64 `json:"time_points"`    // 时间点序列
	SolutionPath  []float64 `json:"solution_path"`  // 解路径
	DerivativePath []float64 `json:"derivative_path"` // 导数路径（二阶方程）
	MethodUsed    string    `json:"method_used"`    // 使用的求解方法
	Stability     string    `json:"stability"`      // 数值稳定性
...
func (c *ODESolverCalculator) solveWithAdams(params *ODEParams) (*ODEResult, error) {
	...
	for t < params.TimeRange {
		...
		if n >= 4 {
			// 使用前4个点进行预测
			yNew := solutionPath[n-1] + params.TimeStep*(
				55*c.evaluateFirstOrder(params.Equation, timePoints[n-1], solutionPath[n-1]) -
